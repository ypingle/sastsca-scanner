    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            for line in file:
                ...
                # Skip editable installs, options, and constraints files
                if line.startswith('-e ') or line.startswith('-') or line.startswith('-c '):
                    continue

                # Skip requirements files (requirements often separate dev from prod)
...
def parse_pyproject_toml(file_path: str) -> List[Tuple[str, str]]:
    """Parse pyproject.toml file to extract only production package dependencies."""
    try:
        ...
        # Check for poetry dependencies (only production, not dev)
        if "tool" in data and "poetry" in data["tool"]:
            # Get main dependencies (skip dev-dependencies)
            ...
            # Explicitly check we're NOT including dev-dependencies
            # Poetry keeps them separate, unlike the others
...
def parse_package_json(file_path: str) -> List[Tuple[str, str]]:
    """Parse package.json file to extract only production npm dependencies."""
    try:
...
def parse_pom_xml(file_path: str) -> List[Tuple[str, str]]:
    """Parse Maven pom.xml file to extract only production dependencies."""
    try:
        ...
        for dep in root.findall(".//maven:dependencies/maven:dependency", ns):
            ...
            # Skip test, provided, and system scopes - they're not production dependencies
            if scope is not None and scope.text in ["test", "provided", "system"]:
                continue
...
def parse_csproj(file_path: str) -> List[Tuple[str, str]]:
    """Parse .NET project file (.csproj, .fsproj, etc.) to extract production NuGet package references."""
    try:
        import xml.etree.ElementTree as ET
        ...
        # Look for PackageReference elements, focusing only on production dependencies
        package_refs = root.findall(".//*[@Include]")
        ...
        for ref in package_refs:
            if ref.tag.endswith('PackageReference'):
                ...
                # Skip common development dependencies that might be in the main package references
                dev_dependencies = [
                ...
                if package and not any(dev_dep in package for dev_dep in dev_dependencies):
                    dependencies.append((package, version))
...
def find_manifest_files(folder_path: str, exclude_patterns: List[str] = None) -> Dict[str, List[str]]:
    ...
    # Add common directories to exclude if not already specified
    common_excludes = ["node_modules", ".git", "__pycache__", "venv", "env", ".venv", ".env"]
    for common in common_excludes:
        if common not in exclude_patterns:
            exclude_patterns.append(common)
    
    for root, dirs, files in os.walk(folder_path):
        # Filter out excluded directories to prevent further recursion
        dirs[:] = [d for d in dirs if d not in exclude_patterns and not any(
            fnmatch.fnmatch(d, pattern) for pattern in exclude_patterns)]
        
        # Skip excluded paths
        if any(pattern in root for pattern in exclude_patterns) or any(
            fnmatch.fnmatch(root, pattern) for pattern in exclude_patterns):
            continue
...
def scan_folder(folder_path: str, exclude_patterns: List[str] = None, include_dev: bool = False) -> List[Dict[str, Any]]:
    ...
    print(f"{'Including' if include_dev else 'Excluding'} development dependencies")
    if exclude_patterns:
        print(f"Excluding patterns: {', '.join(exclude_patterns)}")
    
    manifest_files = find_manifest_files(folder_path, exclude_patterns)
    ...
    for registry, files in manifest_files.items():
        for file_path in files:
            ...
            for package, version in dependencies:
                # Skip test-only packages if we're excluding dev dependencies
                if not include_dev and is_dev_dependency(package, registry):
                    continue
    ...
    for dep in all_dependencies:
        ...
        if key not in unique_deps or (dep['version'] != "Unknown" and unique_deps[key]['version'] == "Unknown"):
            unique_deps[key] = dep
    ...
    print(f"Found {len(batch_packages)} unique {'production' if not include_dev else ''} dependencies. Checking security...")
...
def is_dev_dependency(package: str, registry: str) -> bool:
    """Check if a package is likely a development dependency based on naming patterns."""
    # Common development dependency patterns by registry
    dev_patterns = {
        "pypi": [
            "test", "mock", "pytest", "coverage", "tox", "pylint", "flake8", "mypy", 
            "black", "isort", "dev", "debug", "doc", "sphinx", "unittest"
        ],
        "npm": [
            "eslint", "prettier", "webpack", "babel", "jest", "mocha", "chai", "karma",
            "typescript", "tslint", "storybook", "cypress", "test", "dev-", "gulp", "grunt"
        ],
        "maven": [
            "junit", "mockito", "testng", "assertj", "hamcrest", "jacoco", "surefire",
            "failsafe", "test", "checkstyle", "spotbugs", "findbugs"
        ],
        "nuget": [
            "test", "xunit", "nunit", "mstest", "moq", "coverlet", "fluentassertions"
        ]
    }
    
    patterns = dev_patterns.get(registry, [])
    package_lower = package.lower()
    
    return any(pattern.lower() in package_lower for pattern in patterns)
...
def main():
    ...
    folder_parser.add_argument("--format", "-f", choices=["json", "csv"], default="csv",
                              help="Output format (default: csv)")
    folder_parser.add_argument("--exclude-pattern", "-e", action="append", default=[],
                              help="Exclude patterns (e.g., 'node_modules', 'test'). Can be specified multiple times.")
    folder_parser.add_argument("--dev", action="store_true", 
                              help="Include development dependencies (default: only production dependencies)")
